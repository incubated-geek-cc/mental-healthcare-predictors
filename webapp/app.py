...
from flask import Flask,render_template,session,request,jsonify,copy_current_request_context
...
# SERVER SIDE
# init variables for model training here
@app.route("/")
def index():
    return render_template("index.html")

# init connection to socket server
custom_namespace="/train_model"

# shall be sending all messages via my_event channel
@sio.on("my_event", namespace=custom_namespace)
...
def train_model_request():
	...
	print("train_model_request done")



@sio.on("profile_and_predict", namespace=custom_namespace)
def profile_and_predict(msg):
	msg_data=msg["data"]
	session["receive_count"]=session.get("receive_count", 0)+1
	...
	emit("prediction", { "data": treatment_likelihood })

# client sends request to disconnect from server
@sio.on("disconnect_request",namespace=custom_namespace)
def disconnect_request():
    @copy_current_request_context
    def can_disconnect():
        disconnect()
    session["receive_count"] = session.get("receive_count", 0)+1
    emit("my_response", {"data": "Client is now disconnected from SocketServer.", "count": session["receive_count"]},callback=can_disconnect)
